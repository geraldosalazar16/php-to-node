...
const maiserController = require('../controllers/maiser.controller');

/**
 * Render post income page
 */
router.get('/', function(req, res, next) {
  res.render('postmaiser', {authenticated: req.session.authenticated});
});
...
async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        ...
        return res.render('postmaiser', data);
    }
    const response = await maiserController.create(req.session.user.userId, req.body);
    const data = Object.assign({}, {authenticated: req.session.authenticated});
    let finalData;
    if (response.result === 'success') {
        finalData = Object.assign({}, data, {message: 'Maiser created successfully'});
    } else {
        finalData = Object.assign({}, data, {message: response.error});
    }
    res.render('postmaiser', finalData);
});
